...
Original file is located at
    https://colab.research.google.com/drive/1c2XNcgcsvSuJrxPqxyvfyfzHf_9bUQ0r
"""
...
def FLT_LIST(COND=None, LIST=['CID']):
...
def SR(Rout='QFILTER', QVAR=[], RNG=[], LIST=['CID']):
...
def MULTI(Rout='QFILTER', QVAR=[], QEX=[],LIST=[]):
    ...
    FLT_LIST(condition1, ['CID', Rout] + QVAR + QEX+LIST)

    # Invalid Punches
    print(f"{QVAR[0]} - Invalid Punches:")
    condition2 = (df['QCount2'] != NR) & (df[Rout] == 1)
    FLT_LIST(condition2, ['CID', Rout] + QVAR + QEX+LIST)
...
def GRID_CHECK(Rout='QFILTER', QVAR=[], CVAR=[], COD=[], LIST=[]):
    ...
    if len(CVAR)==0:
        ...
        FLT_LIST(COND=condition1, LIST=['CID', Rout] + QVAR)

        # Step 2: Filter OFF Check
        if ((df[Rout] == 0).sum(axis=0)>0):
           print(f"{QVAR[0]} - Filter OFF Check:")
           condition2 = ((df[Rout] == 0) | df[Rout].isna()) & (df[QVAR].isna().sum(axis=1) != NR)
           FLT_LIST(COND=condition2, LIST=['CID', Rout] + QVAR + LIST)
    ...
    if len(CVAR)!=0:
        ...
        FLT_LIST(COND=(df['err'] > 0), LIST=['CID', 'err'] + CVAR + QVAR+LIST)
...
def RANK_CHECK(Rout='QFILTER', QVAR=[], MAXR=0, MINR=None):
    ...
    if MINR is not None:
        ...
        FLT_LIST(COND=condition_min_rank, LIST=['CID', Rout, 'QCount1'] + QVAR)
    ...
    FLT_LIST(COND=condition_invalid, LIST=['CID', Rout, 'QCount1'] + QVAR)
    ...
    FLT_LIST(COND=condition_duplicate, LIST=['CID', Rout, 'QCount1'] + QVAR)

    # 4. Filter OFF Check
    if ((df[Rout] == 0).sum(axis=0)>0):
    	print(f"{QVAR[0]} - Filter OFF Check:")
    	condition_filter_off = ((df[Rout] == 0) | (df[Rout].isna())) & (df[QVAR].isna().sum(axis=1) != NR)
    	FLT_LIST(COND=condition_filter_off, LIST=['CID', Rout] + QVAR)
